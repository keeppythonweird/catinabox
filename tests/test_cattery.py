...
def test__add_cats__succeeds(cattery_clien):
    c = cattery.Cattery()
...
def test__remove_cat__succeeds(cattery_clien):
    c = cattery.Cattery()
...
def test__remove_cat__no_cats__fails(cattery_clien):
    c = cattery.Cattery()
    with pytest.raises(cattery.CatNotFound):
        c.remove_cat("Fluffles")


def test__remove_cat__cat_not_in_cattery__fails(cattery_clien):
    c = cattery.Cattery()
    c.add_cats(["Fluffy"])
    with pytest.raises(cattery.CatNotFound):
